...
40. **Estante do Sábio: contar um livro** (pedido do pai em 22/09; desenho em `docs/LEITURA_LIVROS.md`; os seis ajustes do líder aprovados pelo pai no mesmo dia; **implementado pelo líder em 22/09**): o pai cadastra o livro quando ele entra em casa (título e páginas); a criança pode propor um, mas esse só paga com o ok do pai. Ao terminar, o Heitor conta o livro ao Sábio na Biblioteca — gostou (4 rostos), nota 0-10 opcional, texto com as próprias palavras (mínimo 40 palavras, 80 depois do 5º livro; molde tocável; colar é recusado na hora). O Sábio (`gpt-4o`) julga pelo título + texto (detalhes que só quem leu sabe; começo, meio, fim, opinião com motivo) e faz **uma pergunta de verificação** sobre um detalhe que não está no texto. Texto perfeito demais para uma criança de 10 anos nunca é recusado sozinho: pede reescrita só quando também é genérico; senão aceita e marca para o pai. Aceito paga o gold **que o pai definiu no cadastro** (as páginas só sugerem: curto até 60 páginas 8 · médio até 150 15 · longo 25; um livro curto e denso pode valer mais) + 40 XP, **um resgate por dia**, **cada livro uma vez**. Recusa nunca castiga: diz o que faltou e mantém o texto (3 tentativas por livro por dia). O pai lê tudo no painel (aba Livros), aprova, anula e deixa uma linha que o Sábio entrega. Conquistas: Primeiro livro (1), Cinco livros (5), Doze livros (12). É o primeiro comprovante da decisão 24.
41. **Missão diária paga só XP e gold** (pai, 22/09: "ele está acumulando muitos recursos"): `materialsPerTask` passa de 1 para **0** no padrão da economia (ajustável na Balança, "Materiais por missão"). Material vem do jogo: Baú do Dia, contratos da Mina, Comerciante (10 materiais por 2 gold, 2 vezes por dia) e Fornalha (fundição 3→1, queima 5 madeira → 1 redstone). O bônus da picareta continua somando nas missões (é o que a Ferraria promete). Sem retroativo: o estoque atual fica.
42. **Comerciante invertido** (pai, 22/09: "o mais difícil e valioso pra ele é o gold"): a criança **paga 2 gold e recebe 10 de um material** (madeira, pedra ou ferro; redstone só na Fornalha), até 2 compras por dia; **acaba a venda de material por gold** (gold só nasce da vida real, da prova e do Baú). Pedido 1 do Comerciante vira "Compre 10 materiais do Comerciante" (`merchantBuys`). Extrato: `merchant_buy`.
43. **Aprender a aprender** (pedido do pai em 23/09; aprovado no mesmo dia: "vamos seguir essa linha de raciocínio"; desenho em `docs/APRENDER_A_APRENDER.md`, falas em `docs/conteudo/FALAS_APRENDER.md`): a habilidade central que o jogo desenvolve passa a ser descobrir a resposta quando ainda não sabe, com transferência para a vida real como requisito. Três leis e uma ponte, sem mecânica nova: (1) **a primeira tentativa é dele**: só ela paga e conta para conquista de desempenho; tudo depois é aprendizado, gravado (`supportLevel`) e reconhecido, nunca pago; (2) **a ajuda vem em degraus**, cada degrau uma ferramenta com nome, só em problema de raciocínio, e nunca custa nada; (3) **o erro volta mudado** numa Estante única (prova, Mina, Túnel, Expedição); domínio é acertar sem ajuda na volta; **a ponte**: a mesma ferramenta, a cada quinzena, nas pistas do jogo, no comprovante do dever e num cartão do painel do pai. Muda decisões anteriores: a **23b** (a segunda tentativa só vale em item de raciocínio, com aviso e sem revelar a certa; em fato, explicação direta; sai "Não foi dessa vez"; entra no pacote 11 de 23/09); a **14** (a revisita deixa de pagar o dobro na Biblioteca nível 3, e a "dica grátis" sai porque a dica passa a ser sempre grátis); a Dica do Recado deixa de custar ferro; o molde da reflexão some depois de 10 reflexões aceitas; "palavra dominada" deixa de ser "vista 3 vezes"; a Expedição perde as estrelas de desempenho; nenhuma porcentagem por matéria nem rótulo de habilidade na tela da criança. Lista completa e ordem de entrega (pacote 11, depois AP1 a AP5) em `APRENDER_A_APRENDER.md` §10 e §12.
44. **O tema de amanhã sai** (pedido do pai em 23/09, ao ver o campo "Tema de amanhã" no cartão da Biblioteca). A criança deixa de digitar o tema da história de amanhã: saem o campo do cartão da Biblioteca, o do mapa da Mina, `setThemeRequest` e o uso do `themeRequest` como tema principal dos contratos. O tema vem só da rotação do currículo. A Biblioteca nível 1 passa a ter como efeito a Estante do Sábio (decisão 40). Nos pedidos do Sábio, o capítulo 4 ("Escolha o tema de amanhã 5 vezes") vira "Conte um livro para o Sábio" (`booksRead`), e o capítulo 3 ("Tire 8 de 8") vira "Acerte todas as perguntas da prova" (`quizPerfect`, que volta a ser alcançável com a correção C2 da revisão do pacote 10). Pacote 10c em `REVISAO_ETAPA_2_LANCAMENTO.md`.
...
8. **Exceção aceita na revisão do pacote 10 (23/09):** a categoria fraca que não tem tema livre nos 90 dias e ainda não apareceu na semana traz de volta o tema menos recente dela, com profundidade maior e o ângulo seguinte (`reason: 'categoria fraca'`). Sem isso, os 6 temas de matemática não cobrem o "pelo menos 1 por semana" da regra 4. Rever um assunto fraco por outro ângulo, antes de 90 dias, também é o que a Estante faz (`docs/APRENDER_A_APRENDER.md` §6).
...
| `forgeItemMixFor`, `forgeStepDown`, `forgeTargetFor` (`prompts.ts`), `scramble_longo` (`validators.ts`), teste `forgeMix.test.ts` | etapa-3 p14a | nota da Ferraria de ontem; etiqueta do Recado | alvo e mistura do dia; frase de montar curta no nível 1 | Ferraria: ordem em frase curta; depois de 0 ou 1 acerto, forma (is/are, a/an) |
| `fillAnyArea`, `why_circular`, `dilema_com_certa` (`validateQuestion.ts`); `challengeLine.ts`; `sageSay.ts`; `docs/conteudo/BANCO_OFFLINE_REPROVADAS.md` | etapa-3 p10b | quizBank 14 dias; banco offline; reflexão | prova com 8; prompt mais difícil; fala do Sábio sobre a frase dele | prova v3 (lei do professor) |
| `src/services/quiz/reserveFromRow.ts` (+ teste); `public/data/provaReserva.json` | etapa-3 p10d | banco de reserva escrito pelo líder | vaga da prova e prova offline; no máximo 2 por área | prova v3 (lei do professor) |
...
| `src/components/hero/village/Torre.tsx` (abas Vida real / Conquistas, cartão que se explica), `src/data/achievements.ts` (`rewardFor`, `REAL_LIFE_CATEGORIES`), `src/services/village/achievements.ts` (`BASE_BUILDINGS` sem Campinho, `rewardLine`, `achievementGoldRoom`, `materialForForgeLevel`) + `achievements.test.ts`, pagamento em `villageService.applyVillageStats` (gold com teto semanal, material do nível da Ferraria, linha `achievement` no extrato), `scripts/archive-legacy-achievements.cjs`; removidos `AchievementsBadges.tsx` e `AchievementManager.tsx` | líder, 22/09 (decisão 38) | `village.stats`, `achievementsUnlocked`, `settings/economy.achievementGoldCap` | XP 30/75/150/300; gold 3/6/12 só em Rotina, Agenda, Baú, Biblioteca; `village.stats.achGoldWeek/achGoldWeekKey`; extrato `achievement` | Torre: ver o que falta e o que ganha, sem tabela (`docs/VILA_CONQUISTAS.md`) |
