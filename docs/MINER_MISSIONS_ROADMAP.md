...
14. **Aprender a aprender** (decisão 43, 23/09; `docs/APRENDER_A_APRENDER.md`): (a) a primeira tentativa, sem ajuda, é a única que paga e conta para conquista de desempenho; tudo depois dela é aprendizado, gravado em `supportLevel` e reconhecido na tela, nunca pago; (b) em problema de raciocínio a resposta é o último degrau da escada de ajuda, e nenhuma ajuda custa gold, material ou equipamento; (c) todo erro volta pela Estante única, e "dominado" é acertar sem ajuda na volta; (d) nenhuma tela da criança mostra porcentagem por matéria, nota ou rótulo de habilidade.
...
Acordar na Vila (Placa diz o que há hoje: missões, prova, evento da Agenda, pedido de NPC) → fazer as **Missões** (gold, XP, material do período; a picareta da Ferraria aumenta o material) → **Biblioteca** (prova: gold, XP, esmeralda quando acerta todas; o tema vem da rotação do currículo; a reflexão vai para o Diário; os livros lidos vão para a Estante do Sábio) → **Mina** (contratos de inglês: material raro e gold; a Fornalha melhora o primeiro contrato; a Lanterna mostra o de amanhã) → **Obras e Ferraria** (material vira construção e equipamento; cada nível muda o dia seguinte) → **Baú do Dia** (fechar todas as missões: gold, material, tocha) → **Fechar o dia** (hábitos, "amanhã eu..."; o Sábio responde de manhã) → dormir. Por semana: tochas viram Baú das 7 tochas e troféu; a Cerca protege um dia ruim; o Banco rende o que ficou guardado; os desafios do pai vencem; o pedido do NPC avança. Por temporada: nível vira patente, patente vira raro e cosmético, raro vira o topo da Ferraria, a estrela fica.
...
| Biblioteca (prova) | rotação do currículo (tema do dia), Agenda (provas da escola viram tema), Estante de erros, livros cadastrados pelo pai (Estante do Sábio) | gold, XP, esmeralda; Diário; amizade do Sábio; desafios `quiz_correct`; Mapa de habilidades |
| Mina (inglês) | Fornalha (bônus), Lanterna (amanhã), temas do nível (`englishLevels.ts`), Banco (nada: aprender não é emprego) | material e redstone, gold, XP; amizade do Comerciante; palavras dominadas (Torre); Baú ("+1 na Fornalha 3") |
...
| Escada de ajuda (decisão 43; todo jogo que ensina) | o erro da primeira tentativa em problema de raciocínio; a ferramenta da quinzena; os bancos de `docs/conteudo/FALAS_APRENDER.md` | `supportLevel` no item (decide quando o erro volta na Estante); sinais do painel ("usa o retorno", "pede ajuda"); Caderno do Minerador (ferramenta que ajudou); a linha "descobriu" na tela |
| Estante única (decisão 43; prova, Mina, Túnel, Expedição) | erros e degraus de ajuda dos quatro módulos; "chute e acertou" da Expedição | a revisita de cada módulo (posição 8 da prova, onda do Túnel, contrato do mesmo tipo); barras na Biblioteca; conquista "Minério em barra"; "dominado" (inclusive palavra dominada); sinal "erro que vira acerto" no painel |
| Laço do plano e ponte da quinzena (decisão 43) | "amanhã eu..." de ontem (Fechar o dia); Agenda (lembrete); calendário de ferramentas | fala do Sábio de manhã; tocha de plano e conquista "Plano cumprido"; cartão do painel (pergunta do jantar, pensar em voz alta, missão-problema nos Desafios); pergunta de processo no comprovante do dever; pistas do jogo com o mesmo nome |
...
7. Todo módulo que ensina passa pela pergunta "isto ensina a descobrir quando ainda não sabe?" e segue a invariante 14; nenhuma ideia vira mecânica nova se couber no comportamento de algo que já existe (`docs/APRENDER_A_APRENDER.md`).

## Aprender a aprender: a lente pedagógica (decisão do pai em 23/09/2026)

Pedido do pai: acima de qualquer matéria, o jogo desenvolve no Heitor a capacidade de descobrir a resposta quando ainda não sabe (perceber o erro, trocar de estratégia, planejar, testar, explicar, saber quanto sabe), e **a transferência para fora do app é requisito de projeto**, não bônus. Desenho completo em `docs/APRENDER_A_APRENDER.md`; falas em `docs/conteudo/FALAS_APRENDER.md`. Em uma linha: **três leis e uma ponte**, sem nenhuma mecânica nova.

1. **A primeira tentativa é dele**: só ela paga; o resto é aprendizado, gravado e reconhecido.
2. **A ajuda vem em degraus**: cada degrau é uma ferramenta com nome (entender o pedido, fazer menor, de trás para frente, testar e olhar, riscar o que não pode), só em problema de raciocínio, e nunca custa nada.
3. **O erro volta mudado**: uma Estante única para prova, Mina, Túnel e Expedição; domínio é acertar sem ajuda na volta.
4. **A ponte**: a cada quinzena, a mesma ferramenta nas pistas do jogo, no comprovante do dever e num cartão do painel, com uma pergunta para o jantar e uma situação para o pai pensar em voz alta.

Cada módulo treina um momento (antes, durante ou depois), o Sábio pergunta pouco e com motivo (no máximo três vezes por dia) e é mentor, não fiscal; o "amanhã eu..." do Fechar o dia vira laço (fiz, em parte, não fiz). Ordem de entrega: pacote 11 da Prova v3 (a segunda tentativa com aviso), depois AP1 (laço do plano e ponte), AP2 (escada nos contratos), AP3 (Prova v3 P2), AP4 (Estante única, a partir de meados de outubro) e AP5 (Diário e Caderno).
...
- **Aprender a aprender** (decisão do pai em 23/09; desenho em `docs/APRENDER_A_APRENDER.md`; vale para tudo desta etapa): a segunda tentativa com aviso entra no pacote 11 da Prova v3; depois, na ordem, AP1 laço do plano e ponte da quinzena, AP2 escada de ajuda nos contratos (dica sem custo), AP3 Prova v3 P2 (conta digitada, campo `hint`, "Ache o erro", previsão semanal, o Sábio em voz alta), AP4 Estante única (a partir de meados de outubro) e AP5 Diário como livro e Caderno do Minerador. Os itens abaixo que tocam erro, revisita, reflexão ou Diário seguem esse desenho onde houver conflito.
...
- **O Túnel** (pedido do pai em 19/09; desenho em `docs/JOGO_TUNEL.md`): o jogo de ação da Mina em Phaser, onde cada inimigo traz um desafio da galeria do dia e o golpe certo é a resposta certa; erro custa um coração e traz a Coruja do Sábio com a regra; revisita na onda seguinte e em 3 e 10 dias; material e XP só de primeira, nunca gold. Nasce do motor do Turno na Mina (órfão desde a Etapa 1) e do molde Phaser. Fatia vertical primeiro (um herói, um inimigo, uma descida de inglês), na branch `tunel`, depois do Comerciante v2.
...
**Aprender a aprender** (decisão 43): três mecânicas que treinam sem virar aula, todas opcionais e puxadas pelo pai: **adivinhe o lance do pai** (de vez em quando, antes do lance do pai, ele marca onde acha que o pai vai jogar: antecipação como jogo dentro do jogo); **voltar à posição que virou a partida** e jogar de novo dali (no Lig 4 o computador acha o momento; no xadrez, um motor gratuito no navegador); **o pai pensando em voz alta** na vez dele (o painel lembra o pai, não a criança). Dados: `arena.predictions[]` e a posição crítica. Sem nota e sem análise obrigatória.
...
9. Que momento treina e como a ajuda funciona (decisão 43): antes, durante ou depois; quais degraus da escada existem; o que é performance (paga) e o que é aprendizado (gravado e reconhecido).
10. Onde isso aparece fora do jogo (decisão 43): a situação real em que a mesma ferramenta serve, e como o pai a encontra no painel.
...
- **Relatório semanal v1** já aqui (junto com o Banco): `learning/{uid}` calculado por código toda semana (acerto por categoria da prova, nível adaptativo de Redstone pelos últimos 10 resultados: >= 7 sobe, <= 3 desce; palavras dominadas; reflexões escritas; taxa de poupança); a criança vê um "Mapa de habilidades" na Torre (desde 23/09, decisão 43: o que ela já domina e o que está na pedra, sem porcentagem; a porcentagem fica só no painel). Painel: card "Hoje" no topo (resgates pendentes, metas batidas, desafios vencendo, dias não fechados, uso de IA > 80%, pacotes a aprovar) e abas reagrupadas em 4 grupos (Hoje, Jogo, Conteúdo, Ajustes).
...
- **Lei da Excelência** (`.cursor/rules/lei-excelencia-aaa.mdc`, criada pelo pai em 19/09): padrão indie AAA para tudo que entra no jogo, com evidência em foto nas duas resoluções; vale para o Cursor e para o líder (arte só depois de olhar a prancha; bancos de texto com amostra de 20 % do pai; docs com aceite em foto).
