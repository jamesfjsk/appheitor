...
import { MATERIALS, MATERIAL_LABELS } from '../../../config/englishBase';
import { initialBaseDoc } from '../../../config/englishBase';
import { canBuild, buildUpgrade } from '../../../services/englishBaseService';
      ...
      <div className="mc-panel rounded-lg w-full max-w-3xl max-h-[96vh] overflow-y-auto text-white" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="p-4 space-y-2">
          {tab === 'build' && BUILDINGS.map((b) => {
            ...
            const info = canBuild(fakeBase, b.id);
            return (
              <div key={b.id} className="mc-row rounded p-3 flex items-center gap-3">
                ...
                >
                  {level >= 3 ? 'Máximo' : info.ok ? 'Construir' : 'Falta material'}
                </button>
              </div>
            );
          })}
          {tab === 'gear' && GEAR.map((g) => {
            const current = village.gear[g.slot];
            const done = g.slot === 'pickaxe' ? current >= g.level : current >= 1;
            return (
              <div key={g.id} className="mc-row rounded p-3 flex items-center gap-3">
                <div className="flex-1">
                  <p className="font-bold">{g.label} {done ? '· feito' : ''}</p>
                  <p className="text-xs mc-muted">{g.effect}</p>
                </div>
                <button type="button" disabled={done} className="mc-btn mc-btn-green min-h-[44px] px-4 font-bold" onClick={() => { playClick(); void craftGear(g.id); }}>
                  {done ? 'Feito' : 'Craftar'}
                </button>
              </div>
            );
          ...
          {tab === 'trade' && (
            <div className="mc-card p-4">
              <p className="mb-3">Troca 3 de um material por 1 de outro. Sem gold.</p>
