...
const Torre: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  ...
  useEffect(() => {
    if (!childUid) return;
    void seeAchievements(childUid).catch(() => undefined);
  ...
  }), [buildings, village.gear, village.npcs, village.owned, progress.level]);
      ...
      <div className="mc-modal mc-pop rounded-lg w-full max-w-2xl max-h-[96vh] overflow-y-auto text-white" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="mc-hotbar p-3">
          {([
            ...
            <button key={id} type="button" className={`mc-slot rounded px-3 min-h-[44px] ${tab === id ? 'mc-slot-selected' : ''}`} onClick={() => { playClick(); setTab(id); }}>
              {label}
            </button>
          ))}
        </div>
        <div className="p-4 space-y-3">
          ...
          {tab === 'vida' && <AchievementsBadges />}
          {tab === 'recordes' && (
            <div className="space-y-2">
              <p className="text-sm">Melhor semana: <span className="mc-num" style={{ fontSize: 12 }}>{village.records.weekGold || 0}</span> gold</p>
              <p className="text-sm">Maior sequência de tochas: <span className="mc-num" style={{ fontSize: 12 }}>{village.records.fullDays || village.fullDays}</span></p>
              ...
              {(Object.keys(village.npcs) as NpcId[]).map((id) => {
                ...
                return (
                  <div key={id} className="mc-card p-3">
                    ...
                    {nextQ && n.quest.chapter < 5 && <p className="text-sm">{id}: {nextQ.ask}</p>}
                  </div>
                );
              })}
            </div>
