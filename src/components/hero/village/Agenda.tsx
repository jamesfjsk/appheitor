...
const DatePick: React.FC<{ value: string; onChange: (v: string) => void }> = ({ value, onChange }) => {
  const [open, setOpen] = useState(false);
  const [y, m] = value.split('-').map(Number);
  const first = new Date(y, m - 1, 1).getDay();
  const days = new Date(y, m, 0).getDate();
  const cells = Array.from({ length: first + days }, (_, i) => (i < first ? 0 : i - first + 1));
  const shift = (delta: number) => {
    const next = new Date(y, m - 1 + delta, 1);
    const dim = new Date(next.getFullYear(), next.getMonth() + 1, 0).getDate();
    const day = Math.min(Number(value.slice(8, 10)), dim);
    onChange(`${next.getFullYear()}-${pad(next.getMonth() + 1)}-${pad(day)}`);
  };
  return (
    <div>
      <button type="button" className="mc-btn mc-btn-stone min-h-[44px] px-3 w-full text-left" onClick={() => setOpen((v) => !v)}>
        {fmtDate(value)}
      </button>
      {open && (
        <div className="mc-card p-2 mt-1">
          <div className="flex justify-between items-center mb-2">
            <button type="button" className="mc-btn mc-btn-dark min-h-[36px] px-2" onClick={() => shift(-1)}>‹</button>
            <span className="text-sm font-bold">{MONTHS[m - 1]} {y}</span>
            <button type="button" className="mc-btn mc-btn-dark min-h-[36px] px-2" onClick={() => shift(1)}>›</button>
          </div>
          <div className="grid grid-cols-7 gap-1 mb-1">
            {WEEKDAYS.map((d, i) => <span key={d + i} className="text-sm mc-muted text-center">{d}</span>)}
            {cells.map((d, i) => (
              <button
                key={i}
                type="button"
                disabled={!d}
                className={`min-h-[36px] text-sm ${d && value === `${y}-${pad(m)}-${pad(d)}` ? 'mc-btn mc-btn-gold' : 'mc-btn mc-btn-dark'}`}
                onClick={() => {
                  if (!d) return;
...
const TimePick: React.FC<{ value: string; onChange: (v: string) => void }> = ({ value, onChange }) => {
  const [h, min] = (value || '12:00').split(':').map(Number);
  return (
    <div className="flex gap-2">
      <div className="mc-slot p-1 h-32 overflow-y-auto flex-1">
        {HOURS.map((hour) => (
          <button
            key={hour}
            type="button"
            className={`w-full min-h-[32px] text-sm ${hour === h ? 'mc-btn mc-btn-gold' : 'mc-btn mc-btn-dark'}`}
            onClick={() => onChange(`${pad(hour)}:${pad(Number.isFinite(min) ? min : 0)}`)}
          >
            {pad(hour)} h
          </button>
        ))}
      ...
      <div className="mc-slot p-1 h-32 overflow-y-auto flex-1">
        {MINS.map((minute) => (
          <button
            key={minute}
            type="button"
            className={`w-full min-h-[32px] text-sm ${minute === min ? 'mc-btn mc-btn-gold' : 'mc-btn mc-btn-dark'}`}
            onClick={() => onChange(`${pad(Number.isFinite(h) ? h : 12)}:${pad(minute)}`)}
          >
            {pad(minute)}
          </button>
        ))}
...
const Agenda: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  ...
  return (
    <ChildSheet onClose={onClose} title="Agenda" tabs={tabs} wide="md">
      <div className="p-4 space-y-3">
        {tab === 'hoje' && (
          <>
            ...
            {todayItems.map((i) => {
              const fromDad = i.createdBy === 'admin';
              return (
                <div key={i.id + i.date} className="mc-card p-3 flex justify-between items-center gap-2">
                  <div>
                    <p className="font-bold">{i.title}</p>
                    <p className="text-sm mc-muted">{i.time || 'sem hora'} · {KIND_LABEL[i.kind]}{fromDad ? ' · do pai' : ''}</p>
                  ...
                  <div className="flex flex-wrap gap-1">
                    ...
                    {!fromDad && <button type="button" className="mc-btn mc-btn-stone min-h-[36px] px-2" onClick={() => startEdit(i)}>Editar</button>}
                    {!fromDad && (askDel === i.id ? (
                      <>
                        <button type="button" className="mc-btn mc-btn-red min-h-[36px] px-2" onClick={() => {
                          playClick();
                          void deleteAgendaItem(i.id).then(() => { toast.success('Apagado'); setAskDel(null); });
                        }}>Confirmar</button>
                        <button type="button" className="mc-btn mc-btn-stone min-h-[36px] px-2" onClick={() => setAskDel(null)}>Não</button>
                      </>
                    ) : (
                      <button type="button" className="mc-btn mc-btn-red min-h-[36px] px-2" onClick={() => { playClick(); setAskDel(i.id); }}>Apagar</button>
                    ))}
                  </div>
                </div>
              );
            ...
            {Array.from({ length: 7 }, (_, i) => {
              ...
              return (
                <div key={d} className="mc-card p-2">
                  <p className="text-sm font-bold mb-1">{d.split('-').reverse().slice(0, 2).join('/')}</p>
                  {list.map((x) => <p key={x.id + x.date} className="text-sm">{x.time ? `${x.time} ` : ''}{x.title}</p>)}
                </div>
              );
            })}
          </div>
        ...
        <div className="mc-card p-3 space-y-2">
          ...
          <p className="text-sm mc-muted">Data</p>
          <DatePick value={date} onChange={setDate} />
          <p className="text-sm mc-muted">Hora (24h)</p>
          <TimePick value={time || '12:00'} onChange={setTime} />
          <button type="button" className="mc-btn mc-btn-stone min-h-[36px] px-2" onClick={() => setTime('')}>Sem hora</button>
          <div className="flex flex-wrap gap-1">
            {REMINDS.map((r) => (
