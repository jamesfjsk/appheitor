...
export class FirestoreService {
  ...
  static async processDailySummary(userId: string, date: string): Promise<{
    ...
    try {
      ...
      const existingSummary = await this.getDailyProgress(userId, date);
      if (existingSummary?.summaryProcessed) {
        console.log('📊 Daily summary already processed for:', date);
        return {
          goldPenalty: existingSummary.goldPenalty || 0,
          allTasksBonusGold: existingSummary.allTasksBonusGold || 0,
          totalTasksAvailable: existingSummary.totalTasksAvailable || 0,
          totalTasksCompleted: existingSummary.totalTasksCompleted || 0,
      ...
      // Get completion history for this specific date
      const completionHistory = await this.getTaskCompletionHistory(
        userId, 
        new Date(date), 
      ...
      const totalTasksCompleted = completionHistory.length;
      const tasksNotCompleted = Math.max(0, totalTasksAvailable - totalTasksCompleted);
      ...
      if (totalTasksAvailable > 0) {
        if (totalTasksCompleted >= totalTasksAvailable) {
          ...
          goldPenalty = tasksNotCompleted; // 1 gold per missed task
        }
      }
      
      // Apply changes to user progress
      const progressRef = doc(db, 'progress', userId);
      const progressDoc = await getDoc(progressRef);
      ...
      if (progressDoc.exists()) {
        ...
        if (allTasksBonusGold > 0) {
          ...
          await updateDoc(progressRef, {
            availableGold: newGold,
            totalGoldSpent: (currentProgress.totalGoldSpent || 0) + Math.min(goldPenalty, currentGold),
            updatedAt: serverTimestamp()
          });
        }
      }
      ...
      await this.setDailySummary(userId, date, summary);
  ...
  static async checkAndProcessMissedDays(userId: string): Promise<void> {
    try {
      console.log('🔍 FirestoreService: Checking for missed days for user:', userId);
      ...
      // If no last processed date, start from yesterday
      const startDate = lastProcessedDate || yesterday;
      ...
      while (currentDate < yesterday) {
        ...
        try {
          const summary = await this.processDailySummary(userId, dateString);
          processedDays.push({ date: dateString, summary });
          console.log(`✅ Processed day ${dateString}:`, summary);
        } catch (error) {
      ...
      if (processedDays.length > 0) {
        ...
        console.log(`✅ FirestoreService: Processed ${processedDays.length} missed days for user:`, userId);
      }
      
    } catch (error) {
      console.error('❌ FirestoreService: Error checking missed days:', error);
      throw error;
    }
  }
